...
export class Rules {
  ...
  static taxonomy(values, field = 'permalink', cond = 'and') {
    return new QueryRule('taxons', field, values, 'fixed', cond);
  }
  ...
  static price(...values) {
    ...
    // Create a new Rule
    return new QueryRule('price', undefined, values, 'range');
  }
  ...
  static filter(name, values, cond = 'or') {
    return new QueryRule(name, undefined, values, 'filter', cond);
  }
}
